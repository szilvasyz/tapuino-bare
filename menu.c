...
#define OPTION_MOTOR_CTL 8
#define OPTION_REC_TYPE 9
...
void handle_mode_options() {
  const char* ppitems[] = {S_OPTION_MACHINE_TYPE, S_OPTION_VIDEO_MODE, S_OPTION_SIGNAL, 
                           S_OPTION_KEY_REPEAT, S_OPTION_TICKER_SPEED, S_OPTION_TICKER_HOLD, 
                           S_OPTION_REC_FINALIZE, S_OPTION_REC_AUTO_FINALIZE, 
                           S_OPTION_MOTOR_CTL, S_OPTION_REC_TYPE};
  uint16_t value = 0;
  uint8_t save = 0;
  uint8_t cur_mode = 0;
  
  while (1) {
    cur_mode = handle_select_mode(S_MODE_OPTIONS, ppitems, 10, cur_mode);
    switch (cur_mode) {
      ...
      break;
      case OPTION_MOTOR_CTL:
        value = g_motor_ctl;
        if (handle_option_value(S_OPTION_MOTOR_CTL, &value, 0, 1, 1)) {
          g_motor_ctl = value;
          save = 1;
        }
      break;
      case OPTION_REC_TYPE:
        value = g_record_type;
        if (handle_option_value(S_OPTION_REC_TYPE, &value, 1, 2, 1)) {
          g_record_type = value;
          save = 1;
        }
      break;
